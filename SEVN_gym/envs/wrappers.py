...
def wrap_obs_pano(obs, use_gps_obs, use_visible_text_obs, use_image_obs, use_goal, num_streets):
    h = obs['image'].shape[0]
    w = obs['image'].shape[1]
    ...
    if use_gps_obs:
        # Relative and Absolute gps (4-D input)
        gps_fm = np.array(obs['rel_gps'], dtype=np.float32)
        gps_fm = np.tile(gps_fm, (h, w // len(gps_fm)))
        gps_fm = np.expand_dims(gps_fm, axis=0)
    else:
        gps_fm = np.zeros((1, h, w), dtype=np.float32)
    ...
    if use_visible_text_obs:
        ...
        vsn_fm = np.tile(vsn_fm, (1, w // num_streets))
        vsn_fm = np.pad(vsn_fm, ((0, 0), (0, w % num_streets)), 'constant', constant_values=0)
        vsn_fm = np.tile(vsn_fm, (int(h/2), 1))
        vsn_fm = np.expand_dims(vsn_fm, axis=0)
        ...
        vhn_fm = np.array([obs['visible_text']['house_numbers'][:40],
                           ...
                           obs['visible_text']['house_numbers'][80:120]], dtype=np.float32)
        if w == 224:
            vhn_fm = np.tile(vhn_fm, (1, 5))
            vhn_fm = np.pad(vhn_fm, ((0, 0), (0, 24)), 'constant', constant_values=0)
            vhn_fm = np.tile(vhn_fm, (28, 1))
        else:
            vhn_fm = vhn_fm.flatten()
            vhn_fm = np.tile(vhn_fm, w // 120)
        ...
        vsn_fm = np.zeros((1, h, w), dtype=np.float32)
        vhn_fm = np.zeros((1, h, w), dtype=np.float32)

    if not use_image_obs:
        obs['image'] = np.zeros((3, h, w), dtype=np.float32)

    if use_goal:
        ghn_fm = np.array(obs['mission']['house_numbers'], dtype=np.float32)
        if w == 224:
            ghn_fm = np.tile(ghn_fm, (1, 5))
            ghn_fm = np.pad(ghn_fm, ((0, 0), (0, 24)), 'constant', constant_values=0)
            ghn_fm = np.tile(ghn_fm, (84, 1))
        else:
            ghn_fm = np.tile(ghn_fm, w // 40)
            ghn_fm = np.tile(ghn_fm, (1280, 1))
        ...
        if w == 224:
            gsn_fm = np.tile(gsn_fm, (1, w // num_streets))
            gsn_fm = np.pad(gsn_fm, ((0, 0), (0, w % num_streets)), 'constant', constant_values=0)
            gsn_fm = np.tile(gsn_fm, (h, 1))
        else:
            gsn_fm = np.tile(gsn_fm, (1, w // num_streets))
            gsn_fm = gsn_fm.flatten()
            gsn_fm = np.pad(gsn_fm, (0, int((w % num_streets))), 'constant')
        ...
        ghn_fm = np.zeros((1, h, w), dtype=np.float32)
        gsn_fm = np.zeros((1, h, w), dtype=np.float32)
    out = np.concatenate((np.moveaxis(obs['image'], 2, 0), gps_fm, vsn_fm, vhn_fm, ghn_fm, gsn_fm), axis=0)
    return out