...
'''
Script that runs on panos directly to obtain the mean and standard deviation
of the color channels in the dataset.
'''
...
print(
    f'Red mean: {images[:, :, :, 0].mean()}' +
    f' StD: {images[:, :, :, 0].std()} \n'
    f'Green mean: {images[:, :, :, 1].mean()}' +
    f' StD: {images[:, :, :, 1].std()} \n'
    f'Blue mean: {images[:, :, :, 2].mean()}' +
    f' StD: {images[:, :, :, 2].std()} ')
